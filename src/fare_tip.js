...
function readFareTipData() {
    if (!loaded) {
        d3.json(chosenCabCategory, function(error, data) {
            ...
            loaded = true;
            console.log("selectedData0: " + selectedData.length);
            renderFareChart();
        });
    } else {
        console.log("selectedData1: " + selectedData.length);
        renderFareChart();
    }
}
...
function renderFareChart() {
    ...
    fareDataset.forEach(function(val) {
        ...
        rects.enter()
            ...
            .attr('height', d => -(yFareScale(d[1])-yFareScale(d[0])));
